...
if world_result:
    ...
    if world_result:
        ...
        print("Aucun monde trouvu00e9 dans la base de donnu00e9es.")
        sys.exit(1)

print(f"Utilisation du monde avec l'ID: {world_id}")

# Fonction pour gu00e9nu00e9rer un ID unique au format UUID
def generate_uuid():
...
# Ru00e9cupu00e9rer une location au hasard pour les articles
cursor.execute("SELECT id FROM locations LIMIT 1")
location_result = cursor.fetchone()
default_location_id = location_result[0] if location_result else None
        ...
        try:
            if table_name == 'hardware_items':
                ...
                cursor.execute(f"""
                    ...
                    (id, world_id, name, description, hardware_type, quality, rarity, level, stats, price, is_available, type, is_legal, location_type, location_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (item_id, world_id, item_name, item_description, hardware_type, quality, rarity, level, stats, price, 1, item_type, 1, location_type, default_location_id))
                ...
                cursor.execute(f"""
                    INSERT INTO {table_name} 
                    (id, name, description, software_type, level, features, price, world_id, is_legal, version)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                cursor.execute(f"""
                    INSERT INTO {table_name} 
                    (id, world_id, name, description, item_type, rarity, uses, effects, price, is_available, consumable_type, is_legal, location_type, location_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (item_id, world_id, item_name, item_description, item_type, rarity, uses, effects, price, 1, consumable_type, 1, location_type, default_location_id))
                ...
                cursor.execute(f"""
                    INSERT INTO {table_name} 
                    (id, world_id, name, description, defense, defense_type, slots, weight, durability, mod_slots, rarity, value, location_type, location_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (item_id, world_id, item_name, item_description, defense, defense_type, slots, weight, durability, mod_slots, rarity, price, location_type, default_location_id))
                
            print(f"Article {item_id} de type {item_type} cru00e9u00e9 avec succu00e8s.")
            
        except sqlite3.Error as e:
            print(f"Erreur lors de la cru00e9ation de l'article {item_id} de type {item_type}: {e}")
